...
def build_package_dir(
    ...
    resources_dir = package_dir / variant.resources_dirname
    path_dir = package_dir / variant.path_dirname
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / variant.code_mode_host_name(spec),
        is_windows=spec.is_windows,
    )
    ...
    metadata = {
        ...
        "resourcesDir": variant.resources_dirname,
        "pathDir": variant.path_dirname,
    }
    write_json(package_dir / variant.package_metadata_name, metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    required_dirs = [
        Path("bin"),
        Path(variant.resources_dirname),
        Path(variant.path_dirname),
    ]
    ...
    metadata_path = package_dir / variant.package_metadata_name
    if not metadata_path.is_file():
        raise RuntimeError(f"Missing package metadata: {variant.package_metadata_name}")
    ...
    required_files = [
        ...
        Path("bin") / variant.code_mode_host_name(spec),
        Path(variant.path_dirname) / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path(variant.resources_dirname) / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path(variant.resources_dirname) / "bwrap")
        executable_files.append(Path(variant.resources_dirname) / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path(variant.resources_dirname) / "codex-command-runner.exe",
                Path(variant.resources_dirname) / "codex-windows-sandbox-setup.exe",
            ]
        )
