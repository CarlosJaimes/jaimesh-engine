...
pub enum Command {
    /// Submit a new remote coding task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a remote coding task.
    Status(StatusCommand),
    /// List remote coding tasks.
    List(ListCommand),
    /// Apply the diff for a remote coding task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a remote coding task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in remote coding.
    ...
    /// Git branch to run in remote coding (defaults to current branch).
...
pub struct StatusCommand {
    /// remote coding task identifier to inspect.
...
pub struct ApplyCommand {
    /// remote coding task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// remote coding task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
