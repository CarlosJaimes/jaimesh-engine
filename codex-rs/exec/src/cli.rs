...
#[command(
    version,
    override_usage = "jaimesh exec [OPTIONS] [PROMPT]\n       jaimesh exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running JaiMesh outside a Git repository.
    ...
    /// Do not load the user configuration file; authentication still uses the agent state directory.
